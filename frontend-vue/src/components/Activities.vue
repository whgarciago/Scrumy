<template>
  <div id="Activities"> <!--Div grande que abarca toda la pagina -->
    ...
    <div class = "create-activity-popup">
        <div id="crearActividad" class="ventanaOpciones">
            <h2>Nueva actividad</h2>
            <p>Nombre: </p>
            <input v-model="NombreActividad" type="text" id="nombreActividad">
            <p>descripcion: </p>
            <input v-model="DescripcionActividad" type="text" id="descripcionActividad">
            <button v-on:click="cerrarVentanas(1)" class="cerrarVentana">x</button>
            <button v-on:click="CrearActividad()" class="textoOpcion">Crear</button>
        </div>
    </div>

    <div class = "delete-activity-popup">
        <div id="eliminarActividad" class="ventanaOpciones">
            ...
            <button v-on:click="cerrarVentanas(2)" class="cerrarVentana">x</button>
            <button v-on:click="EliminarActividad()" class="textoOpcion">Eliminar</button>
        </div>
    </div>
    <button v-on:click="isHidden = true" id="CreateActivityButton">+</button>
    <button v-on:click="buttons(2)" id="DeleteActivityButton"><img src="https://img.icons8.com/glyph-neue/50/4a90e2/trash.png"/></button>
...
export default {
  ...
  methods:{
    abrirPopupCrear: function() {
      document.querySelector(".create-activity-popup").classList.add("active");
    },
    cerrarPopupCrear: function() {
      document.querySelector(".create-activity-popup").classList.remove("active");
    },
    abrirPopupBorrar: function() {
      document.querySelector(".delete-activity-popup").classList.add("active");
    },
    cerrarPopupBorrar: function() {
      document.querySelector(".create-activity-popup").classList.remove("active");
    },
  ...
  buttons(){
      if(isHiddenCreate == true){
          document.getElementById("crearActividad").style.visibility = "visible";
      }
      if(isHiddenDelete == true){
          document.getElementById("eliminarActividad").style.visibility = "visible";
      }
  },
  cerrarVentanas(){
      if(isHiddenCreate == true){
          document.getElementById("crearActividad").style.visibility = "hidden";
      }else{
          document.getElementById("eliminarActividad").style.visibility = "hidden";
      }
  },
  CrearActividad(){
